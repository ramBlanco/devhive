...
3. **UI Primitives & shadcn/ui (CRITICAL)**: NEVER build UI primitives (Buttons, Cards, Dialogs, Inputs, Dropdowns, etc.) manually from scratch. 
   - If the project uses `shadcn/ui` (indicated by a `components.json` file), check what is already installed first.
   - If a required primitive is missing, you MUST use the CLI (e.g., `npx shadcn@latest add <component>`) or the `shadcn` skill to install it FIRST.
   - ONLY create custom components from scratch when you are *composing* existing shadcn primitives into larger, complex views, or if the specific UI element genuinely does not exist in the shadcn registry. Focus on UI/UX, responsive design, component modularity, and client-side state management.
